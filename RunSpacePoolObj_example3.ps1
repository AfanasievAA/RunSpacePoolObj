...
# Example #3 of RunSpacePoolObj usage
...
# With error immitation in a some scripts
Try {
...
$RunSpacePoolObj.ScriptBlock = {
    param ([int]$pauseTimer, $text)
    # If pause times more than 25 - immitating an error
    if ($pauseTimer -gt 25) {
        # Immitate of error. No such function
        Get-NoFunctionError
    }
    # Sleeping random ammount of time
    Start-Sleep -Seconds $pauseTimer
    # Write-Information returns a value from Job
    Write-Information "$($pauseTimer) seconds passed. $($text)"
}
# Initialize should be run after any changing in RunSpacePoolObj parameters
...
$RunSpacePoolObj.Jobs | Select-Object Id,Started,Ended,State,Arguments,Information,Errors | Format-List
